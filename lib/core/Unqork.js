...
class Unqork {
    ...
    connect = async () => {
        try {
            ...
            this.unqorkAccessTokenURI = unqorkAccessTokenURI;
            ...
            this.expiresIn = access_data.data.expires_in + Math.floor(Number(new Date() / 1000));
    ...
    // Check the Access Token if it needs to be refreshed or not and return the same
    checkAccessToken = async () => {

        if (!this.access_token || !this.expiresIn || this.expiresIn < Math.floor(Number(new Date() / 1000)) + 240) {
            const refresh_token = await axios.post(this.unqorkAccessTokenURI,
                {
                    grant_type: "password",
                    username: this.client_id,
                    password: this.client_secret
            ...
            this.access_token = refresh_token.data.access_token;
            this.expiresIn = refresh_token.data.expires_in + Math.floor(Number(new Date() / 1000));
        }
    ...
    /**
     * Creates an Axios Instance to be used across the library
     * @returns Axios Unqork Instance
     */
    ...
    /**
     * Execute is a library specific function which will use the Axios instance 
     * to make a network call to Unqork API
     * 
     * @param {String} method String Method - get, post, patch, put, delete
     * @param {String} uri String URI
     * @param {Object} request_body Object 
     * @returns 
     */
    ...
    executeModule = async (moduleId, request_body) => {
        ...
        // Service URI
        let uri = `/modules/${moduleId}/execute`;
        try {
    ...
    executeViaProxy = async (method, moduleId, request_body, query) => {
        // Service URI
        let uri = this.platformExpressURL + `/modules/${moduleId}/api`;
    ...
    getModuleComponents = async (moduleId, locale) => {
        ...
        let uri = this.platformExpressURL + `/modules/${moduleId}/components`;
     ...
     * according to the api-query-params library. For example, //query/people/all?age=33 
    ...
    getRowsFromTable = async (table, filter, limit) => {
        let uri = this.platformExpressURL + `/query/${table}/all`;
     ...
     * filter conditions, and sorting, according to the api-query-params library. For example, //query/people/distinct?distinct=name&age=33 will result in 
     * [{"name":"Alice"},{"name":"Bob"}].
     * 
     * @param {String} table Required
    ...
    getDistinctValuesFromTable = async (table, distinctField, filter) => {
        let uri = this.platformExpressURL + `/query/${table}/distinct?distinct=${distinctField}`;
    ...
    getModuleSubmissions = async (moduleId, queryParams) => {
        ...
        let uri = this.platformExpressURL + `/modules/${moduleId}/submissions`;
    ...
    getWorkflowSubmissions = async (workflowId, queryParams) => {
        ...
        let uri = this.platformExpressURL + `/workflows/${workflowId}/submissions`;
     ...
     * Example: //system/getSubmissions?moduleId=<module1>&moduleId=<module2>&fields=name,age&age=33
     * 
    ...
    listSubmissionsDashboard = async (moduleId, queryParams) => {
        ...
        let uri = this.platformExpressURL + `/system/getSubmissions?moduleId=${moduleId}`;
    ...
    getSubmission = async (schemaType, schemaId, submissionId, queryParams) => {
        ...
        let uri = this.platformExpressURL + '/';
    ...
    createWorkflowSubmission = async (workflowPath, requestBody) => {
        let uri = this.platformExpressURL + `/workflow-execute/${workflowPath}`;
